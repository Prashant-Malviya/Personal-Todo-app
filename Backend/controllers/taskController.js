...
const addTask = async (req, res) => {
    const { title, description, type, dueDate } = req.body; // Include type and dueDate
    const userId = req.user.id;
    const user = await userModel.find({ _id: userId });
    const newTask = new taskModel({ title, description, type, dueDate, userId, completed: false }); // Include type and dueDate
    newTask.save()
        .then(() => {
